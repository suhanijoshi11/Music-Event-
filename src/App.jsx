  ...
  return (
    <>
      <div className="max-w-md mx-auto p-4 pb-20 space-y-5 relative z-10">
        ...
        <div className="bg-white/86 border border-sky-200 rounded-3xl p-5"><div className="flex items-center justify-between"><div><h3 className="text-sm font-black text-slate-900">{t('organisingTeam')}</h3><p className="text-[11px] text-slate-500 mt-1">{t('meetPeople')}</p></div><Sparkles className="w-5 h-5 text-sky-700" /></div><div className="grid grid-cols-2 gap-3 mt-4">{organizerPeople.slice(0, 2).map((person) => <PersonMiniPhotoCard key={person.id || person.name} person={person} />)}</div><button onClick={() => openPeople('anchor')} className="w-full mt-4 bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 rounded-xl py-2.5 text-xs font-black transition">{t('knowAnchorsTeam')}</button></div>
