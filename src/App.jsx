...
const GANA_CREDIT = 'by GANA Music Lovers';
const DEFAULT_CREATOR_PROFILE = {
  name: 'Suhani',
  credit: GANA_CREDIT,
  line1: 'B.Tech CSE student — Second Year',
  line2: 'Sangit Upvisharad • Bharatnatyam Visharad',
  line3: 'Designed and developed for Tofani Vayra 9.',
  photo_url: '',
};
...
async function buildMemoryCardBlob({ name, photoUrl, score, tag, language = 'en', creatorProfile = DEFAULT_CREATOR_PROFILE }) {
  ...
  ctx.fillText(creatorProfile?.credit || GANA_CREDIT, 70, 116);
  ctx.fillStyle = '#0f766e';
  ctx.font = '700 20px Arial';
  ctx.fillText(language === 'hi' ? 'आपकी म्यूज़िक नाइट याद' : language === 'gu' ? 'તમારી મ્યુઝિક નાઇટ યાદ' : 'YOUR MUSIC NIGHT MEMORY', 70, 148);
  ...
  ctx.fillText(language === 'hi' ? 'आप सिर्फ दर्शक नहीं, इस संगीत का हिस्सा थे।' : language === 'gu' ? 'તમે માત્ર પ્રેક્ષક નહોતા, આ સંગીતનો એક ભાગ હતા.' : 'You were part of the music, not just the audience.', 70, 585);

  ctx.fillStyle = '#334155';
  ctx.font = '600 18px Arial';
  ctx.fillText(`Made with ❤️ by ${creatorProfile?.name || 'Suhani'}`, 70, 615);
...
export default function App() {
  ...
  const [creatorProfile, setCreatorProfile] = useState(DEFAULT_CREATOR_PROFILE);
  ...
  const refreshLiveData = async () => {
    ...
    if (!snapshotsResult.error) {
      const creatorSnapshot = (snapshotsResult.data || []).find((row) => row.snapshot?.__type === 'creator_profile');
      if (creatorSnapshot?.snapshot?.profile) setCreatorProfile({ ...DEFAULT_CREATOR_PROFILE, ...creatorSnapshot.snapshot.profile });
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-5 relative z-10">
      <div className="w-full max-w-md">
        <div className="bg-white/88 border border-sky-200 rounded-3xl p-7 shadow-2xl text-center backdrop-blur-xl">
          ...
          <p className="text-xs font-black text-sky-700 mt-1">{GANA_CREDIT}</p>
...
function AudienceHome({ session, language = 'en', nowPlaying, supabaseInteractions, supabaseVotes, supabaseReactions, people, audienceProfile, onAudiencePhotoUpload, onChangeNickname, creatorProfile = DEFAULT_CREATOR_PROFILE }) {
      ...
      <div className="max-w-md mx-auto p-4 pb-20 space-y-5 relative z-10">
        <div className="bg-white/88 border border-sky-200 rounded-2xl p-4 backdrop-blur shadow-md flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-black text-sky-700 mt-0.5">{EVENT_DISPLAY_NAME} • {creatorProfile?.credit || GANA_CREDIT}</p>
          </div>
          <button
        ...
        <div className="bg-white/80 border border-slate-200 rounded-3xl p-4 text-center shadow-sm">
          <div className="flex justify-center mb-3">
            <div className="w-14 h-14 rounded-full overflow-hidden bg-sky-50 border border-sky-200 flex items-center justify-center text-lg">
              {creatorProfile?.photo_url ? <img src={creatorProfile.photo_url} alt={creatorProfile.name || 'Creator'} className="w-full h-full object-cover" /> : '❤️'}
            </div>
          </div>
          <p className="text-sm font-black text-slate-900">Made with ❤️ by {creatorProfile?.name || 'Suhani'}</p>
          <p className="text-[11px] text-slate-500 mt-1">{creatorProfile?.line1 || DEFAULT_CREATOR_PROFILE.line1}</p>
          <p className="text-[11px] text-slate-500">{creatorProfile?.line2 || DEFAULT_CREATOR_PROFILE.line2}</p>
          <p className="text-[11px] text-slate-500">{creatorProfile?.line3 || DEFAULT_CREATOR_PROFILE.line3}</p>
          <p className="text-[10px] font-black text-sky-700 mt-2">{creatorProfile?.credit || GANA_CREDIT}</p>
        </div>
      </div>
...
function AdminDashboard({ dbData, updateDatabase, activeTab, setActiveTab, onLogout, nowPlaying, supabasePerformances, supabaseInteractions, supabaseVotes, supabaseReactions, audienceCount, supabaseAudience, people, analyticsSnapshots, supabaseMemoryCards, onRefresh, creatorProfile }) {
  const tabs = [
...
function AdminCreatorTab({ creatorProfile = DEFAULT_CREATOR_PROFILE, onRefresh }) {
  const [form, setForm] = useState({ ...DEFAULT_CREATOR_PROFILE, ...creatorProfile });
  const [photoFile, setPhotoFile] = useState(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    setForm({ ...DEFAULT_CREATOR_PROFILE, ...creatorProfile });
  }, [creatorProfile]);

  const setField = (field, value) => setForm((current) => ({ ...current, [field]: value }));

  const saveCreator = async (event) => {
    event.preventDefault();
    ...
    try {
      ...
      if (photoFile) {
        const upload = await uploadEventMedia(photoFile, `creator/${APP_EVENT_ID}`, 'creator');
        if (upload.error) throw upload.error;
        photoUrl = upload.url;
      }

      const profile = {
        name: form.name.trim() || 'Suhani',
        credit: form.credit.trim() || GANA_CREDIT,
      ...
      const { error: deleteError } = await supabase
        ...
        .delete()
        .eq('event_id', APP_EVENT_ID)
        .contains('snapshot', { __type: 'creator_profile' });
      if (deleteError) console.warn('Old creator profile cleanup skipped:', deleteError.message);

      const { error } = await supabase.from('event_analytics_snapshots').insert({
        event_id: APP_EVENT_ID,
        event_name: EVENT_DISPLAY_NAME,
        snapshot: { __type: 'creator_profile', profile },
      });
      ...
      alert(`Could not save creator profile: ${error.message}`);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-2xl space-y-5">
      <div className="bg-white/84 border border-sky-200 rounded-3xl p-5 shadow-lg">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-11 h-11 rounded-2xl bg-pink-50 border border-pink-200 flex items-center justify-center text-pink-600"><Heart className="w-5 h-5" /></div>
          <div><h2 className="text-lg font-black text-slate-900">Creator / Event Credits</h2><p className="text-xs text-slate-500">Separate from Performers, Musicians, Anchors and Organising Team.</p></div>
        </div>
        <form onSubmit={saveCreator} className="space-y-4">
          <div className="flex items-center gap-4 bg-sky-50 border border-sky-100 rounded-2xl p-4">
            <div className="w-16 h-16 rounded-full overflow-hidden bg-white border border-sky-200 flex items-center justify-center text-xl shrink-0">
              {form.photo_url ? <img src={form.photo_url} alt={form.name || 'Creator'} className="w-full h-full object-cover" /> : '❤️'}
            </div>
            <div className="min-w-0"><p className="text-xs font-black text-slate-800">Creator Photo</p><input type="file" accept="image/*" onChange={(event) => setPhotoFile(event.target.files?.[0] || null)} className="w-full mt-2 text-[11px] text-slate-500 file:mr-2 file:rounded-lg file:border-0 file:bg-white file:px-2.5 file:py-1.5 file:text-[10px] file:font-black file:text-sky-700" /></div>
          </div>
          <FormInput label="Name" value={form.name} setValue={(v) => setField('name', v)} placeholder="Suhani" />
          <FormInput label="Credit" value={form.credit} setValue={(v) => setField('credit', v)} placeholder="by GANA Music Lovers" />
          <FormInput label="Line 1" value={form.line1} setValue={(v) => setField('line1', v)} placeholder={DEFAULT_CREATOR_PROFILE.line1} />
          <FormInput label="Line 2" value={form.line2} setValue={(v) => setField('line2', v)} placeholder={DEFAULT_CREATOR_PROFILE.line2} />
          <FormInput label="Line 3" value={form.line3} setValue={(v) => setField('line3', v)} placeholder={DEFAULT_CREATOR_PROFILE.line3} />
          <button type="submit" disabled={saving} className="w-full bg-sky-600 hover:bg-sky-500 disabled:opacity-60 text-white font-black py-3 rounded-xl text-sm flex items-center justify-center gap-2"><Save className="w-4 h-4" />{saving ? 'Saving…' : 'Save Creator Profile'}</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/84 border border-sky-200 rounded-3xl p-5 shadow-lg">
        ...
        <div className="space-y-2 mt-4">
          {analyticsSnapshots.filter((snapshot) => snapshot.snapshot?.__type !== 'creator_profile').length ? analyticsSnapshots.filter((snapshot) => snapshot.snapshot?.__type !== 'creator_profile').map((snapshot) => (
            <div
              key={snapshot.id}
